...
void testApp::setup(){
    ...
    //youtube_search_string = createYoutubeSearchString("cats");
    ...
    */
    ...
    //manually add videos for now
    
    mVideoPlayerController.addVideoPlayer();
    mVideoPlayerController.addVideoURL("videos/kill_la_kill.mp4");

    mVideoPlayerController.addVideoPlayer();
    mVideoPlayerController.addVideoURL("videos/azumanga.mp4");
    
    mVideoPlayerController.addVideoPlayer();
    mVideoPlayerController.addVideoURL("videos/haruhi.mp4");
    
    mVideoPlayerController.addVideoPlayer();
    mVideoPlayerController.addVideoURL("videos/meteor.mp4");
    
    mVideoPlayerController.addVideoPlayer();
    mVideoPlayerController.addVideoURL("videos/asuka.mp4");
    
    cout << "num of video players: " << mVideoPlayerController.getNumberOfVideos();
    ...
    mVideoCubeController.setup(mVideoPlayerController);

    //load miku movie
    mikuPlayer.setPixelFormat(OF_PIXELS_RGBA);
    mikuPlayer.loadMovie("videos/Miku_2.mov");
    mikuPlayer.play();
}
...
void testApp::draw(){
    ...
    //draw our fbo with some cute shaders on top xoxxo
    //bloomShader.begin();
...
void testApp::drawIntoFBO(){
    ...
    mikuPlayer.draw(ofGetWindowWidth()/2-mikuPlayer.getWidth()/2, ofGetWindowHeight()-mikuPlayer.getHeight(), mikuPlayer.getWidth()/2, mikuPlayer.getHeight()/2);
...
void testApp::setupUI(){
    ...
    gui->addWidget(new ofxUILabel("Video Cubes", OFX_UI_FONT_LARGE));
    //gui->addWidget(new ofxUISlider(304,16,0.0,100.0,100.0,"FADE AMOUNT"));
    gui->addSlider("FADE AMOUNT", 0.0, 100.0, 100.0);
    gui->addToggle("TOGGLE PLAYBACK", false);

    /*
    gui->addWidget(new ofxUIToggle(32, 32, false, "FULLSCREEN"));
...
void testApp::guiEvent(ofxUIEventArgs &e){
    ...
    if(e.widget->getName() == "FADE AMOUNT")
    {
        ...
        fadeAmnt = slider->getScaledValue();
