...
void videoCube::setup( videoPlayerController _vpc ){
    //cout << "setup sphere" << endl;
    sphere.set(mRadius, 50);
    sphere.mapTexCoordsFromTexture(_vpc.mVideoPlayers[videoNumber].getTextureReference());
...
void videoCube::update( bool _flatten ){
    ...
    sphere.setPosition(mLoc.x, mLoc.y, mLoc.z);
...
void videoCube::draw(videoPlayerController _vpc){
    ...
    /*
		...
		ofFill();
        ofEnableDepthTest();
        ofDrawSphere(mLoc.x, mLoc.y, mLoc.z, mRadius);
    ...
    shader.begin();
    shader.setUniformTexture("tex0", _vpc.mVideoPlayers[videoNumber].getTextureReference(), 0);
